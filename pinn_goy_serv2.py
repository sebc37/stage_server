    ...
    for j in range(mode_min,mode_max):
        for i in range(t_min,nb_line):
            if (np.random.random()<=ratio):
                ...
                X_posx.append(j)
                X_posy.append(i)
                X_value.append((X_subset[i,j]-mean_mode[j])/std_mode[j])
...
Data_filtered ,Var_mode,Std_mode,perc,Data_train,mean = filter_mode(Data_shell,5,10,2,0.001,123456)
...
class Train_PINN():
    ...
    def train(self):
        ...
        for iteration in tqdm.tqdm(range(self.nbr_iteration)):
            ...
            u_t = torch.zeros_like(u_pd)
            for i in range((k_max-k_min)*Npts):
                if i%Npts !=0:
                    u_t[i] = (u_pd[i+1]-u_pd[i])/dt
                if i== (k_max-k_min)*Npts-1:
                    u_t[i] = (u_pd[i]-u_pd[i-1])/dt
                else:
                    u_t[i] = (u_pd[i+1]-u_pd[i-1])/2*dt
            ...
            for i in range (k_min_collocation,k_max-2):
                GOY_physics[:,i-k_min_collocation] = (u_t[:,i] -K[i]*u_pd[:,i+1]*u_pd[:,i+2] +K[i]*eps/lmb*u_pd[:,i-1]*u_pd[:,i+1] - K[i]*((eps-1)/lmb**2)*u_pd[:,i-2]*u_pd[:,i-1] + nu*K[i]*K[i]*u_pd[:,i])**2 # à confirmer
            loss_physics = self.w_4*torch.mean(GOY_physics).to(device)
...
for i in range(U.shape[1]):
    ...
    plt.ylabel('Velocity')
    plt.legend()
    plt.savefig(f"/Odyssey/private/s26calme/code_stage/GOY-main/prediction_u{i}.png")
...
plt.ylabel('Losses')
plt.legend()
plt.savefig("/Odyssey/private/s26calme/code_stage/GOY-main/losses.png")
