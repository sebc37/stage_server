        ...
        if self.iteration:
            ...
            for iteration in tqdm.tqdm(range(self.nbr_iteration)):
                ...
                if iteration <= self.change_optimizer:
                    ...
                    ################# CALCUL LOSS PHYSIC ################################
                    # CRÉER UNE FONCTION LOSS PHYSIC 
                    # CRÉER UNE FONCTION/CODE DE SAUVEGARDE DU MEILLEUR POIDS AVANT RETROPROPAGATION(test loss avant retro propagation) 
                    # CRÉER 2 BOUCLE FOR AVEC OPTIMIZER ADAM PUIS OPTIMIZER SS-BROYDEN

                    # optim adam si pic de loss intense ==> sauvegarde
                    # reprise de l'entrainement avec ss-broyden
...
Data_shell = data[debut:debut + int(0.2*Nmax),:] # on garde que la partie réelle de chaque shell
...
# ── Variance log ──────────────────────────────────────────────────────────────
# mean_shell = np.mean(Pred,axis=1) # moyenne de chaque shell sur le temps
# pred_centered = Pred - mean_shell[:, np.newaxis] # on centre les données de chaque shell
mean_shell = np.mean(np.sqrt(Pred[0::2,:] ** 2 + Pred[1::2,:]**2), axis=1)
module_pred = np.sqrt(Pred[0::2,:] ** 2 + Pred[1::2,:]**2)
for i in range (int(n/2)):
    module_pred[i,:] = (module_pred[i,:]- mean_shell[i])**2

fig, ax = plt.subplots()
ax.semilogy(range(n // 2), np.mean(Exa[0::2,:] ** 2 + Exa[1::2,:]**2,  axis=1), label='Vérité terrain')
ax.semilogy(range(n // 2), np.mean(module_pred,axis=1) , label='Prédiction') #((Pred[0::2,:] ** 2 + Pred[1::2,:]**2) - np.mean(Pred[0::2,:] ** 2 + Pred[1::2,:]**2, axis=1))**2
...
# mean = np.mean(Pred[0::2,:] ** 4 + Pred[1::2,:]**4, axis=1)
# real_kur = module_pred = Pred[0::2,:] ** 4 + Pred[1::2,:]**4
# for i in range (int(n/2)):
#     real_kur[i,:] = real_kur[i,:]- mean[i]
