...
class Train_PINN():
    ...
    def train(self):
        ...
        for iteration in tqdm.tqdm(range(self.nbr_iteration)):
            ...
            u_pd_all = model(FULL_GRID[:,0:2].to(device))
            u_exa_all = FULL_GRID[:,2:3].to(device)
            ...
            #Total Loss
            total_loss =  loss_boundary_conditions #+ loss_physics #loss_initital_conditions +  loss_collocation_conditions
            total_loss.backward()
            self.optimizer.step()
...
U_pred = model(FULL_GRID[:,0:2].to(device))
